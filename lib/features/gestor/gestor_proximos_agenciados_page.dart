...
import "../recruiter/recruiter_onboarding_central_page.dart"
    show onboardingSteps;

(String, Color) _deriveStatus(Map<String, dynamic> checklist, bool linked) {
  if (checklist["entrou_agencia"] == true) {
    return linked
        ? ("Em onboarding", Colors.greenAccent)
        : ("Aguardando entrada", Colors.orangeAccent);
  }
  if (checklist["convite_aceito"] == true)
    return ("Convite aceito", Colors.amber);
...
class GestorProximosAgenciadosPage extends StatefulWidget {
  ...
  @override
  State<GestorProximosAgenciadosPage> createState() =>
      _GestorProximosAgenciadosPageState();
}

class _GestorProximosAgenciadosPageState
    extends State<GestorProximosAgenciadosPage> {
  ...
  Future<List<Map<String, dynamic>>> _load() async {
    ...
    final gestorRows = await client
        .from("lead_onboarding_gestores")
        .select("lead_id")
        .eq("manager_id", userId);
    final leadIds = (gestorRows as List)
    ...
    final leads = await client
        .from("leads")
        .select(
          "id, name, tiktok_username, phone, category_interest, recruiter_id, converted_streamer_id",
        )
        .inFilter("id", leadIds);
    final leadsList = (leads as List).cast<Map<String, dynamic>>();

    final recruiterIds = leadsList
        .map((l) => l["recruiter_id"] as String)
        .toSet()
        .toList();
    final recruiters = recruiterIds.isEmpty
        ? []
        : await client
              .from("managers")
              .select("id, login_email")
              .inFilter("id", recruiterIds);
    final recruiterMap = {
      for (final r in recruiters) r["id"] as String: r["login_email"] as String,
    };

    final result = <Map<String, dynamic>>[];
    for (final l in leadsList) {
      var checklist = await client
  ...
  void _openDetail(Map<String, dynamic> item) {
    showDialog(
      context: context,
      builder: (context) => _ProximoAgenciadoDetailDialog(
        leadId: item["leadId"] as String,
        leadName: item["name"] as String,
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            "Streamers que ja tem voce como gestor, mas ainda estao concluindo a entrada na agencia.",
            style: TextStyle(
            ...
            child: FutureBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                if (list.isEmpty)
                  return const Center(
                    child: Text(
                      "Nenhum streamer a caminho no momento.",
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      shape: RoundedRectangleBorder(
                        ...
                        side: BorderSide(color: status.$2.withOpacity(0.6)),
                      ),
                      child: ListTile(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            if (lastUpdate != null)
                              Text(
                                "Ultima atualizacao pelo recrutador: " +
                                    DateTime.parse(
                                      lastUpdate,
...
class _ProximoAgenciadoDetailDialog extends StatefulWidget {
  ...
  const _ProximoAgenciadoDetailDialog({
    required this.leadId,
    required this.leadName,
  });

  @override
  State<_ProximoAgenciadoDetailDialog> createState() =>
      _ProximoAgenciadoDetailDialogState();
}

class _ProximoAgenciadoDetailDialogState
    extends State<_ProximoAgenciadoDetailDialog> {
  ...
  Future<Map<String, dynamic>> _load() async {
    ...
    final lead = await client
        .from("leads")
        .select(
          "name, tiktok_username, phone, category_interest, origin, created_at, recruiter_id, converted_streamer_id",
        )
        .eq("id", widget.leadId)
        .single();
              ...
              return SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    if ((lead["tiktok_username"] as String?)?.isNotEmpty ==
                        true)
                      _infoRow("@TikTok", lead["tiktok_username"] as String),
                    if ((lead["phone"] as String?)?.isNotEmpty == true)
                      _infoRow("WhatsApp", lead["phone"] as String),
                    if ((lead["category_interest"] as String?)?.isNotEmpty ==
                        true)
                      _infoRow(
                        "Categoria",
                        lead["category_interest"] as String,
                      ),
                    ...
                    if (handoff != null) ...[
                      const SizedBox(height: 16),
                      const Text(
                        "Informacoes do streamer (transferencia)",
                        style: TextStyle(
                          color: Colors.white,
                          fontWeight: FontWeight.bold,
                        ),
                      ...
                      if ((handoff["niche"] as String?)?.isNotEmpty == true)
                        _infoRow("Nicho", handoff["niche"] as String),
                      if ((handoff["available_days"] as String?)?.isNotEmpty ==
                          true)
                        _infoRow(
                          "Dias disponiveis",
                          handoff["available_days"] as String,
                        ),
                      if ((handoff["available_hours"] as String?)?.isNotEmpty ==
                          true)
                        _infoRow(
                          "Horarios disponiveis",
                          handoff["available_hours"] as String,
                        ),
                      if ((handoff["previous_experience"] as String?)
                              ?.isNotEmpty ==
                          true)
                        _infoRow(
                      ...
                      if ((handoff["objectives"] as String?)?.isNotEmpty ==
                          true)
                        _infoRow("Objetivos", handoff["objectives"] as String),
                      if ((handoff["attention_points"] as String?)
                              ?.isNotEmpty ==
                          true)
                        _infoRow(
                          "Pontos de atencao",
