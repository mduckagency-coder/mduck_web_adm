...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<Map<String, dynamic>> _load() async {
    ...
    final me = await client
        .from("managers")
        .select("agency_id, financial_role")
        .eq("id", userId)
        .maybeSingle();
    ...
    for (final m in (monthlyRows as List)) {
      ...
      cumulativeDaysMap[sid] =
          (cumulativeDaysMap[sid] ?? 0) + ((m["days_live"] as int?) ?? 0);
    }
    ...
    for (final p in (profiles as List)) {
      ...
      final cumulativeDays =
          (cumulativeDaysMap[p["id"]] ?? 0) + daysCurrentMonth;
      ...
      totalHorasPassado +=
          (past?["hours_live"] as num?)?.toDouble()?.toInt() ?? 0;

      final lastLiveText = p["last_live_at"] as String?;
      if (lastLiveText != null) {
        final daysSince = now
            .toUtc()
            .difference(DateTime.parse(lastLiveText))
            .inDays;
    ...
    final allByDiamonds = [...streamers]
      ..sort((a, b) => (b["diamonds"] as int).compareTo(a["diamonds"] as int));
    final allByHoras = [...streamers]
      ..sort((a, b) => (b["hours"] as double).compareTo(a["hours"] as double));
    final topDiasAcumulado = [...streamers]
      ..sort(
        (a, b) =>
            (b["cumulativeDays"] as int).compareTo(a["cumulativeDays"] as int),
      );
    ...
    final streamersComLive = streamers
        .where((s) => (s["days"] as int) > 0)
        .length;
    final liveOpenPct = streamers.isEmpty
        ? 0.0
        : (streamersComLive / streamers.length) * 100;
    final streamers80k = streamers
        .where((s) => (s["diamonds"] as int) >= streamers80kThreshold)
        .length;

    // Recrutamento = criadores totais que entraram na agencia no mes, sem o
    // filtro is_active (quem entrou e saiu no mesmo mes ainda "entrou"), e
    // agrupado por tiktok_group_name -- varias contas TikTok do mesmo grupo
    // (duo/familia) contam como um unico criador agenciado, nao uma por conta.
    ...
    final novosAgenciados = novosAgenciadosKeys.length;
  ...
  Widget _rankSection(
    ...
    Color color,
    List<dynamic> all,
    String Function(Map) valueBuilder, {
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          color: medalColor.withOpacity(i < 3 ? 1 : 0.2),
                        ),
                        child: Text(
                          "#" + (i + 1).toString(),
                          style: TextStyle(
                            color: i < 3 ? Colors.black : Colors.white54,
                            fontWeight: FontWeight.bold,
                            fontSize: 11,
                          ),
                        ),
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          _goalCard(
                            ...
                            Icons.person_add,
                            Colors.blueAccent,
                            (d["novosAgenciados"] as int).toDouble(),
                            (d["recruitmentGoal"] as int?)?.toDouble(),
                          ...
                          _goalCard(
                            ...
                            Icons.live_tv,
                            Colors.orangeAccent,
                            d["liveOpenPct"] as double,
                            d["liveOpenPctGoal"] as double?,
                            (v) => v.toStringAsFixed(0) + "%",
                          ),
                          _goalCard(
                            "Streamers acima de " +
                                streamers80kThreshold.toString(),
                            Icons.diamond_outlined,
                            Colors.greenAccent,
                            (d["streamers80k"] as int).toDouble(),
                            (d["streamers80kGoal"] as int?)?.toDouble(),
                            (v) => v.toStringAsFixed(0),
                          ),
                        ],
                      ),
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          _metricCard(
                            Icons.access_time,
                            "Horas de live (mes atual)",
                            (d["totalHorasAtual"] as double).toStringAsFixed(0),
                            Colors.orangeAccent,
                            change: pctText(d["horasChange"]),
                          ),
                          _metricCard(
                            Icons.people,
                            "Streamers ativos",
                            d["totalStreamers"].toString(),
                            Colors.greenAccent,
                          ),
                          _metricCard(
                            Icons.person_add,
                            "Novos agenciados (mes)",
                            d["novosAgenciados"].toString(),
                            Colors.blueAccent,
                          ),
                          _metricCard(
                            Icons.warning_amber,
                      ...
                      Wrap(
                        ...
                        children: [
                          _comparisonChart(
                            "Diamantes: mes passado vs atual",
                            (d["totalDiamondsPassado"] as int).toDouble(),
                            (d["totalDiamondsAtual"] as int).toDouble(),
                            const Color(0xFF7A0BD4),
                          ),
                          _comparisonChart(
                            "Horas: mes passado vs atual",
                            d["totalHorasPassado"] as double,
                            d["totalHorasAtual"] as double,
                            Colors.orangeAccent,
                          ),
                        ],
                      ),
                      const SizedBox(height: 28),
                      const Text(
                        "Rankings (prioridade: Diamantes e Horas - com lista completa)",
                        style: TextStyle(
                          color: Colors.white54,
                          fontSize: 12,
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          _rankSection(
                            ...
                            Icons.access_time,
                            Colors.orangeAccent,
                            d["allByHoras"] as List,
                            (m) =>
                                (m["hours"] as double).toStringAsFixed(0) + "h",
                          ),
                          _rankSection(
                            "Dias Ativos (acumulado, todos os meses)",
                            Icons.calendar_today,
                            Colors.greenAccent,
                            d["topDiasAcumulado"] as List,
                            (m) => m["cumulativeDays"].toString() + "d",
                            fullScroll: false,
                          ),
                        ],
                      ),
...
class _EditGoalsDialogState extends State<_EditGoalsDialog> {
  late final _diamondsController = TextEditingController(
    text: widget.diamondsGoal?.toStringAsFixed(0) ?? "",
  );
  late final _recruitmentController = TextEditingController(
    text: widget.recruitmentGoal?.toString() ?? "",
  );
  late final _liveOpenPctController = TextEditingController(
    text: widget.liveOpenPctGoal?.toStringAsFixed(0) ?? "",
  );
  late final _streamers80kController = TextEditingController(
    text: widget.streamers80kGoal?.toString() ?? "",
  );
  bool _saving = false;
  String? _error;
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await client.from("agency_goals").upsert({
        ...
        "live_open_pct_goal": double.tryParse(
          _liveOpenPctController.text.trim(),
        ),
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(
                ...
                style: const TextStyle(color: Colors.white),
                decoration: const InputDecoration(
                  labelText: "Meta de recrutamento (novos agenciados)",
                  labelStyle: TextStyle(color: Colors.white54),
                ),
              ),
              const SizedBox(height: 8),
              TextField(
                controller: _liveOpenPctController,
                keyboardType: TextInputType.number,
                style: const TextStyle(color: Colors.white),
                decoration: const InputDecoration(
                  labelText: "Meta de abertura de live (%)",
                  labelStyle: TextStyle(color: Colors.white54),
                ),
              ),
              const SizedBox(height: 8),
              TextField(
                controller: _streamers80kController,
                keyboardType: TextInputType.number,
                style: const TextStyle(color: Colors.white),
                decoration: InputDecoration(
                  labelText:
                      "Meta de streamers acima de " +
                      streamers80kThreshold.toString(),
