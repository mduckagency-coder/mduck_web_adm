...
class _ProgramaDetailPageState extends State<ProgramaDetailPage> {
  ...
  Future<Map<String, dynamic>> _load() async {
    ...
    final program = await client
        ...
        .select(
          "*, program_managers:development_program_managers(manager:managers(login_email))",
        )
    ...
    for (final r in progressList) {
      ...
      } else if (evalStageKeys.contains(r["stage_key"]) &&
          r["outcome"] == null) {
        pendingCount++;
      } else {
        inProgressCount++;
        ...
        child: SizedBox(
          ...
          child: Image.network(
            ...
            errorBuilder: (context, error, stack) => Container(
              ...
              child: Icon(
                programIcon(program["icon_key"] as String?),
                color: color,
                size: 20,
              ),
            ),
    ...
    return Scaffold(
      ...
      body: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          final progressFraction = goalTotal == 0
              ? 0.0
              : (completedCount / goalTotal).clamp(0.0, 1.0);
          final status = programStatus(
            hasStages: hasStages,
            status: programStatusValue,
          );
          final startDate = program["start_date"] != null
              ? DateTime.parse(program["start_date"] as String)
              : null;
          final endDate = program["end_date"] != null
                          ...
                          children: [
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                const SizedBox(width: 4),
                                Text(
                                  monthLabel(_selectedYear, _selectedMonth),
                                  style: const TextStyle(
                                    color: Colors.white70,
                                    fontSize: 12,
                                    fontWeight: FontWeight.bold,
                                ...
                                if (_selectedYear > DateTime.now().year ||
                                  ...
                                  const Text(
                                    "(mes futuro -- mostrando dados do mes atual, ainda nao da pra prever)",
                                    style: TextStyle(
                                      color: Colors.amber,
                                      fontSize: 10,
                            ...
                            Wrap(
                              ...
                              children: [
                                _metaChip(
                                  ...
                                  _responsiblesLabel(program),
                                ),
                                _metaChip(
                                  Icons.groups_outlined,
                                  activeCount.toString() + " participantes",
                                ),
                                _metaChip(
                                  Icons.emoji_events_outlined,
                                  completedCount.toString() + " concluidos",
                                ),
                                if (startDate != null)
                                  _metaChip(
                                    Icons.event,
                                ...
                                if (endDate != null)
                                  _metaChip(
                                    Icons.event_busy,
                                    "Ate " +
                                        endDate.toLocal().toString().substring(
                                          0,
                                          10,
                                        ),
                                  ),
