...
class ProgramaParticipantesTab extends StatefulWidget {
  ...
  final int year;
  final int month;
  const ProgramaParticipantesTab({
    super.key,
  ...
  @override
  State<ProgramaParticipantesTab> createState() =>
      _ProgramaParticipantesTabState();
}
...
class _ProgramaParticipantesTabState extends State<ProgramaParticipantesTab> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final progressRows = await client
          ...
          .select(
            "id, streamer_id, stage_key, outcome, completed_at, started_at, tags, pinned_note, archived_at, manual_override",
          )
      ...
      final snapshotsRaw = await fetchStreamerSnapshotsByIds(
      ...
      for (final p in progressList) {
        ...
        p["eligibility"] = snapshot != null
            ? evaluateEligibility(snapshot, criteria)
            : null;
        p["progress"] = snapshot != null
            ? progressFraction(snapshot, criteria)
            : 0.0;
      }

      await Future.wait(
        progressList.map((p) async {
          final streamerId = p["streamer_id"] as String;
          try {
  ...
  /// andamento" (neutro, ainda nao e urgente). Limiares fatorados em
  /// program_eligibility_service.dart (criticoThreshold/proximoMetaThreshold)
  /// pra reusar na aba Fluxo sem duplicar o numero magico.

  String _category(Map<String, dynamic> row) {
    if (row["completed_at"] != null) {
      final outcome = row["outcome"] as String?;
      return (outcome == "aprovado" || outcome == "graduado")
          ? "concluidos"
          : "atrasados";
    }
    if ((row["pendingAwards"] as int) > 0) return "premiacoes_pendentes";
    if (row["isEvaluationStage"] == true && row["outcome"] == null)
    ...
    if (progress >= proximoMetaThreshold) return "proximos_meta";
    if (progress < criticoThreshold) return "critico";
    return "em_andamento";
  }
  ...
  String _nextAction(Map<String, dynamic> row, String category) {
    switch (category) {
      ...
      case "critico":
        final eligibility = row["eligibility"] as EligibilityResult?;
        final gap = eligibility != null && eligibility.gaps.isNotEmpty
            ? eligibility.gaps.first
            : "distante da meta";
        return "Critico: " + gap;
      default:
        final eligibility = row["eligibility"] as EligibilityResult?;
        if (eligibility != null && eligibility.gaps.isNotEmpty)
          return eligibility.gaps.first;
        return "Em andamento";
    }
  }
  ...
  List<Map<String, dynamic>> get _filteredSorted {
    ...
    list = list
        ...
        .toList();
    if (_filter != "todos")
      list = list.where((r) => r["_category"] == _filter).toList();
    ...
    bool isFavorite(Map<String, dynamic> r) =>
        ((r["tags"] as List?)?.cast<String>() ?? const <String>[]).contains(
          tagInfo(ParticipantTag.destaque).$1,
        );

    list.sort((a, b) {
      // Favoritos (marcador Destaque) sempre primeiro, independente do
      // criterio de ordenacao escolhido.
      final favoriteCompare = (isFavorite(a) ? 0 : 1).compareTo(
        isFavorite(b) ? 0 : 1,
      );
      ...
      switch (_sort) {
        ...
        case "progresso":
          return (b["progress"] as double? ?? 0).compareTo(
            a["progress"] as double? ?? 0,
          );
        case "data_entrada":
          return ((b["started_at"] as String?) ?? "").compareTo(
            (a["started_at"] as String?) ?? "",
          );
        default:
          return (sa?.displayName ?? "").toLowerCase().compareTo(
            (sb?.displayName ?? "").toLowerCase(),
          );
      }
    });
    return list;
  ...
  Future<void> _sendNote(
    ...
    final text = await _promptText(
      isMessage ? "Enviar mensagem" : "Registrar observacao",
    );
    ...
    try {
      ...
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "Salvo! Fica visivel no CRM e no Historico do programa.",
            ),
          ),
        );
    } catch (e) {
  ...
  Future<void> _moveToNextProgram(Map<String, dynamic> row) async {
    ...
    if (nextKey == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Este programa nao tem um proximo programa configurado.",
          ),
        ),
      );
      return;
    ...
    final ok = await confirmAction(
      ...
      message:
          "Mover " +
          (snapshot?.displayName ?? "este streamer") +
          " para o proximo programa?",
      confirmLabel: "Mover",
    );
    if (!ok) return;
    try {
      await createProgramCardIfNeeded(
        phaseKey: nextKey,
        streamerId: row["streamer_id"] as String,
      );
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Movido para o proximo programa.")),
        );
    } catch (e) {
      if (mounted) showProgramasActionError(context, e);
    }
  ...
  Future<void> _addStreamerManually() async {
    ...
    for (final s in selected) {
      try {
        ...
        if (criteria.membershipMode == "faixa") {
          await reactivateOrCreateCard(
            phaseKey: _phaseKey,
            streamerId: streamerId,
          );
          await client
              .from("streamer_phase_progress")
              .update({"manual_override": "add"})
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            added.toString() + " streamer(s) adicionado(s) ao programa.",
          ),
        ),
      );
      _load();
  ...
  Future<void> _addTag(Map<String, dynamic> row, ParticipantTag tag) async {
    final current = ((row["tags"] as List?)?.cast<String>() ?? <String>[])
        .toList();
  ...
  Future<void> _pickTagToAdd(Map<String, dynamic> row) async {
    final current = ((row["tags"] as List?)?.cast<String>() ?? <String>[])
        .toSet();
    final available = ParticipantTag.values
        .where((t) => !current.contains(tagInfo(t).$1))
        .toList();
  ...
  Future<void> _bulkNote({required bool isMessage}) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            "Aplicado para " + success.toString() + " participante(s).",
          ),
        ),
      );
      setState(() {
  ...
  Future<void> _bulkMarkAwardsDelivered() async {
    ...
    final ok = await confirmAction(
      ...
      message:
          "Marcar todas as premiacoes pendentes de " +
          count.toString() +
          " participante(s) como entregues?",
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            delivered.toString() +
                " premiacao(oes) marcada(s) como entregue(s).",
          ),
        ),
      );
      setState(() {
  ...
  Future<void> _bulkExport() async {
    ...
    if (mounted)
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Lista copiada para a area de transferencia."),
        ),
      );
  }
  ...
  Future<void> _pickColumns() async {
    final current =
        (_visibleColumns ?? _optionalColumns.map((c) => c.$1).toList()).toSet();
              ...
              children: _optionalColumns.map((c) {
                return CheckboxListTile(
                  ...
                  onChanged: (v) => setDialogState(
                    () => v == true ? current.add(c.$1) : current.remove(c.$1),
                  ),
    ...
    final columns = _optionalColumns
        .map((c) => c.$1)
  ...
  Widget _buildParticipantsTable(List<Map<String, dynamic>> list) {
    final visible =
        _visibleColumns ?? _optionalColumns.map((c) => c.$1).toList();
    final columnKeys = _optionalColumns
        .map((c) => c.$1)
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: badgeInfo.$3.withOpacity(0.15),
                  borderRadius: BorderRadius.circular(6),
                  border: Border.all(color: badgeInfo.$3),
                ),
                child: Text(
                  badgeInfo.$1,
                  style: TextStyle(
              ...
              if (inactiveDays != null)
                Container(
                  ...
                  child: Text(
                    inactiveDays.toString() + " dias +inativo",
                    style: const TextStyle(
          ...
          rows: list.map((row) {
            ...
            return DataRow(
              cells: [
                ...
                ...columnKeys.map(
                  (k) => DataCell(cellFor(k, row, snapshot, category)),
                ),
                    ...
                    children: [
                      if (!_showArchived)
                        IconButton(
                          ...
                          tooltip: "Mensagem",
                          onPressed: () => _sendNote(row, isMessage: true),
                        ),
                      IconButton(
                        iconSize: 16,
                        icon: const Icon(
                        ...
                        onPressed: () => openParticipantPanel(
                          context,
                          program: widget.program,
                          row: row,
                          nextAction: _nextAction(row, category),
                        ),
                      ),
                      IconButton(
                        iconSize: 16,
                        ...
                        tooltip: "Observacao",
                        onPressed: () => _sendNote(row, isMessage: false),
                      ),
                      if (!_showArchived)
                        IconButton(
                          iconSize: 16,
                          ...
                          tooltip: "Mover",
                          onPressed: () => _moveToNextProgram(row),
                        ),
                      if (!_showArchived)
                        IconButton(
                          iconSize: 16,
                          ...
                          onPressed: () => _removeFromProgram(row),
                        ),
                    ],
                  ),
                ),
  ...
  Widget _buildStatsFooter() {
    final snapshots = _rows
        .map((r) => r["snapshot"] as StreamerSnapshot?)
        .whereType<StreamerSnapshot>()
        .toList();
    ...
    final heartMeValues = snapshots
        .map((s) => s.heartMe)
        .whereType<num>()
        .toList();
    final avgHeartMe = heartMeValues.isEmpty
        ? 0.0
        : heartMeValues.reduce((a, b) => a + b) / heartMeValues.length;
    ...
    final completedCount = _rows
        .where(
          (r) =>
              r["archived_at"] != null &&
              (r["outcome"] == "aprovado" || r["outcome"] == "graduado"),
        )
        .length;
  ...
  Widget build(BuildContext context) {
    final list = _loading || _errorMessage != null
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              ...
              TextButton.icon(
                ...
                label: Text(
                  _showArchived ? "Voltar aos ativos" : "Ver arquivados",
                  style: const TextStyle(color: Colors.white70, fontSize: 12),
                ),
              ),
              if (!_showArchived)
          ...
          Wrap(
            ...
            children: [
              ...(_showArchived ? _archivedFilterOptions : _activeFilterOptions)
                  .map((f) {
              ...
              child: Wrap(
                ...
                children: [
                  ...
                  ElevatedButton.icon(
                    onPressed: _selectedIds.isEmpty
                        ? null
                        : _bulkMarkAwardsDelivered,
                    icon: const Icon(Icons.card_giftcard, size: 14),
                    label: const Text(
                      "Marcar premiacao entregue",
                      style: TextStyle(fontSize: 12),
                    ),
                  ),
                  ElevatedButton.icon(
            ...
            child: _loading
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final snapshot = row["snapshot"] as StreamerSnapshot?;
                      final badgeInfo = streamerBadgeInfo(_badgeFor(category));
                      final progress = (row["progress"] as double? ?? 0).clamp(
                        0.0,
                        1.0,
                      );
                      ...
                      final inactiveDays = _inactiveDaysIfStale(
                        category,
                        snapshot,
                      );
                            ...
                            children: [
                              ...
                              Wrap(
                                ...
                                children: [
                                  ...
                                  _statChip(
                                    "Diamantes",
                                    snapshot?.diamonds.toString() ?? "-",
                                    delta?.diamondsDelta,
                                  ),
                                  _statChip(
                                    "Heart Me",
                                    snapshot?.heartMe?.toString() ?? "-",
                                    delta?.heartMeDelta,
                                  ),
                                ],
                              ),
            ...
            Padding(
              ...
              child: Center(
                child: TextButton(
                  ...
                  child: Text(
                    "Carregar mais (" +
                        (list.length - _visibleCount).toString() +
                        " restantes)",
                  ),
                ),
              ),
            ),
          if (!_loading && _errorMessage == null && _rows.isNotEmpty)
            _buildStatsFooter(),
        ],
      ),
    );
